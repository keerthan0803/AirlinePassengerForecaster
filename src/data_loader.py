...
class DataLoader:
    ...
    def load_data(self):
        ...
        try:
            ...
            raise IOError(f"Failed to read CSV dataset at {self.file_path}. Details: {e}")

        # Validate required columns
        if "Month" not in df.columns:
            raise KeyError(f"Invalid dataset structure: 'Month' column missing. Found columns: {list(df.columns)}")
        if "Passengers" not in df.columns:
            raise KeyError(f"Invalid dataset structure: 'Passengers' column missing. Found columns: {list(df.columns)}")
        ...
        try:
            ...
            raise ValueError(f"Failed to convert 'Month' column to datetime format. Details: {e}")
